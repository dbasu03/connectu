...
const Chatroom = ({ setShowChatroom }) => {
  ...
  useEffect(() => {
    ...
    const messagesRef = collection(db, "chatrooms", activeChatroom.id, "messages");
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        const storageRef = ref(storage, `chatroom-media/${activeChatroom.id}/${Date.now()}_${file.name}`);
      ...
      await addDoc(collection(db, "chatrooms", activeChatroom.id, "messages"), messageData);
    ...
    <>
      ...
      <div className={`chatroom-container ${isChatOpen ? "active" : ""}`}>
        ...
        <div className="chat-box">
          <h2 className="chat-header">Rooms</h2>
